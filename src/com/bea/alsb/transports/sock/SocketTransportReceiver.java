...
public class SocketTransportReceiver implements Runnable {
   ...
   *
  */
  static class As400WorkerThread implements Runnable {
    private Socket clientSocket;
    private SocketTransportEndPoint endPoint;
    private int timeout;
    private boolean enableNagleAlgorithm;
    ...
    public void run() {
      try {
        ...
        if (length > 0) {
          ...
          din.read(resbyte);
          msg = "Reply from Server: " + new String(resbyte, Charset.forName("IBM285"));
        }
        final SocketInboundMessageContext inboundMessageContext =
          new SocketInboundMessageContext(endPoint, as400Socket, msgId, msg);
