...
import com.bea.alsb.transports.sample.test.TestUtil;
...
public class TestClient {
  ...
  public TestClient(String msg, String host, int port, int threadCt,
                    String fileEnc, String messageDelimiter) {
    this.host = host;
    this.port = port;
  ...
  public static void main(String[] args) {
    ...
    if (args.length < 3) {
      ...
      log(
        "Usage is : java -Dfile-encoding=<file-encoding> " +
          ...
          "<host-name> <port>  <thread-ct> <message-file-location> <message-delimiter> \n" +
    ...
    if (args.length > 4) {
      messageDelimiter = args[4];
    }
    TestClient testClient = new TestClient(msg, host, port, threadCt, fileEnc, messageDelimiter);
    ...
    public void run() {
      try {
        ...
        try {
          ...
          outputStream.write(TestUtil.decodeMessageDelimiter(messageDelimiter).getBytes(fileEnc));
          outputStream.flush();
          ...
          String delimiter = new String(TestUtil.decodeMessageDelimiter(messageDelimiter).getBytes(resEnc));
          while (true) {
